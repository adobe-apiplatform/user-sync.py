...
[instruções avançadas de configuração]({{< ref "advanced_configuration#accessing-groups-in-other-organizations" >}})
abaixo.
...
[Recomendações de segurança]({{< ref "deployment_best_practices#security-recommendations" >}}) deste
documento.
...
Consulte a seção [Recomendações de segurança]({{< ref "deployment_best_practices#security-recommendations" >}}) para
...
[Proteção de contas específicas contra exclusão do User Sync]({< ref "advanced_configuration#protecting-specific-accounts-from-user-sync-deletion" >}})
...
[Configurar sincronização de acesso ao produto]({{< ref "setup_and_installation#set-up-product-access-synchronization" >}}).
