...
Si necesita configurar el acceso a grupos de Adobe en otras organizaciones que le han concedido el acceso, puede incluir archivos de configuración adicionales. Para obtener más información, consulte las [instrucciones de configuración avanzada]({{< ref "advanced_configuration#acceso-a-grupos-de-otras-organizaciones" >}}) a continuación.
...
Consulte la sección [Consideraciones de seguridad]({{< ref "deployment_best_practices#recomendaciones-de-seguridad" >}}) para obtener más información sobre cómo almacenar la contraseña de forma más segura la versión 2.1 de User Sync o posterior.
...
2. Ejecute User Sync. (`./user-sync --users all --process-groups --adobe-only-user-action exclude`)
3. Compruebe que los usuarios de prueba de Adobe Admin Console se hayan actualizado para reflejar la nueva pertenencia a configuración de producto.
