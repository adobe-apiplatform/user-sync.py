...
Si vous devez configurer l’accès aux groupes Adobe dans d’autres organisations qui vous y ont habilité, vous pouvez inclure des fichiers de configuration supplémentaires. Pour en savoir plus, reportez-vous à la section [Instructions de configuration avancée]({{< ref "advanced_configuration#instructions-de-configuration-avancée" >}}) ci-dessous.
...
Voir la section [Recommandations de sécurité]({{< ref "deployment_best_practices#recommandations-de-sécurité" >}}) pour plus de détails sur la façon de stocker le mot de passe avec plus de sécurité dans User Sync version 2.1 ou ultérieure.
