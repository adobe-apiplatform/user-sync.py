...
By default, only users whose identity type is Enterprise ID or Federated ID will
be created, removed, or have their group memberships managed by User Sync,
because generally Adobe ID users are not managed in the directory. See the
[description below]({{< ref
"advanced_configuration#managing-users-with-adobe-ids" >}}) under [Advanced
Configuration]({{< ref "advanced_configuration" >}}) if
this is how your organization works.
