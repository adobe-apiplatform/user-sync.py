...
| `--user-filter` _regex\_pattern_ | Limit the set of users that are examined for syncing to those matching a pattern specified with a regular expression. See the [Python regular expression docs](https://docs.python.org/3/library/re.html) for more information. The user name must completely match the regular expression.|
