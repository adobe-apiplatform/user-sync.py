...
the [Configure group mapping]({{< ref "configuring_user_sync_tool#configure-group-mapping" >}}) section
below.
...
2. Get the [latest release](https://github.com/adobe-apiplatform/user-sync.py/releases/latest)
    ...
    * **NOTE:** Releases tagged with the `-noext` edition tag ship with [extension support]({{< ref "advanced_configuration#custom-attributes-and-mappings" >}})
...
define.  See section [Security recommendations]({{< ref "deployment_best_practices#security-recommendations" >}})
for more details.
...
[Security recommendations]({{< ref "deployment_best_practices#security-recommendations" >}}).
