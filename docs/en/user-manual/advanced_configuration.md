...
If you have multiple active directories or sources of identity, the sync tool can be used with all of them at once. This is accomplished with a pseudo connector called the multi directory connector.

The multi directory connector will pull user information from all identity sources and compile a single list of users to sync. This list can be combined with specific directory grouping notation to inform user sync on where to send users on each identity source. This enables the sync tool to map any directory group from any identity source to any group on any target console.

To use the multi sync functionality, specify the “–connector multi” option, and configure user-sync-config.yml to read from multiple directories as shown below.
````yaml
