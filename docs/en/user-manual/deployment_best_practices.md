...
In environments where SSL inspection is enforced at the firewall, the UMAPI client or another connector can encounter the following error or one similar:
...
Or add the command line parameter:

`user-sync.exe [OPTIONS] --no-ssl-cert-verify`

---

This option does NOT apply to the Okta connector, since it depends on a third party library for requests.
