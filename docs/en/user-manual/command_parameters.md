...
| `--ssl-cert-verify`<br />`--no-ssl-cert-verify` | Available in 2.6 or later. Optionl.  Default is `--ssl-cert-verify`. Enable or disables SSL verification of SSL certificates in the requests module.  Can be used when SSL inspection on the firewall interrupts the sync process, throwing an CERTIFICATE_VERIFY_FAILED error.  This does NOT apply to the okta connector. |
